...
elseif (hypermarket_is_woocommerce_activated() && is_shop()):
	...
	if (!empty($get_shop_page_featured_image_url)):
		echo '<!-- Featured Image --><div class="before-layloaded"><div class="featured-image data-background lazyload" data-background="' . esc_url(get_the_post_thumbnail_url(get_option('woocommerce_shop_page_id'))) . '"></div></div><!-- .featured-image -->';
	endif;
else:
	if (has_post_thumbnail()):
		global $post;
		$image_id = get_post_thumbnail_id($post->ID);
		echo '<!-- Featured Image --><div class="before-layloaded"><div class="featured-image data-background lazyload" data-background="' . esc_url(wp_get_attachment_image_url($image_id, 'full')) . '"></div></div><!-- .featured-image -->';
	endif;
endif;